...
<template>
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
    <div class="bg-white p-6 rounded shadow-lg w-80">
      <h2 class="text-lg font-bold mb-4">New Event on {{ props.date ? props.date.toDateString() : '' }}</h2>
      <div class="mb-2">
        <input type="text" v-model="title" placeholder="Title" class="w-full px-2 py-1 border rounded" />
      </div>
      <div class="mb-2">
      ...
      <div class="flex justify-end space-x-2">
        ...
        <button @click="submitEvent" class="px-3 py-1 bg-blue-600 text-white rounded">Submit</button>
      </div>
    </div>
  </div>
</template>
