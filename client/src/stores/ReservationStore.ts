...
export const useReservationStore = defineStore('reservation', {
  ...
  actions: {
    ...
    async updateReservations() {
      ...
      axios
        .get('/api/reservations/venue/' + this.selectedRoom.key)
        .then((response) => {
    ...
    async getReservationTypes() {
      ...
      axios
        ...
        .then((response) => {
          if (response.status === 200) {
            response.data.reservationTypes.forEach(
              (reservationType: { id: string; name: string }) => {
    ...
    async bookReservation(date: Date, event: EventItem) {
      ...
      axios
        .post(`/api/reservations/create`, body)
        .then((response) => {
          if (response.status === 201) {
            const userStore = useUserStore()
            const organizer = {
              key: userStore.userId,
              value: (userStore.title + ' ' || '') + userStore.firstName + ' ' + userStore.lastName,
            }
